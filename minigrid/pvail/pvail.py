...
class PVAILAlgo(BaseAlgo):
    ...
    def __init__(self, protagonist_envs, antagonist_envs, protagonist_acmodel, antagonist_acmodel, discmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95,
        ...
        super(PVAILAlgo, self).__init__(protagonist_envs, antagonist_envs, protagonist_acmodel, antagonist_acmodel, discmodel, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        for _ in range(self.epochs):
            ...
            if True:
                ...
                for i in range(self.disc_recurrence):
                    ...
                    if self.discmodel.recurrent:
                        antagonist_exps_learner, antagonist_exps_memory, antagonist_exps_mu, antagonist_exps_logvar = self.discmodel(antagonist_exps_sb.obs, antagonist_exps_sb.action.to(self.device), antagonist_exps_memory.to(self.device) * antagonist_exps_sb.mask.to(self.device))
                        demos_learner, demos_memory, demos_mu, demos_logvar = self.discmodel(demos_sb.obs, demos_sb.action, demos_memory * demos_sb.mask)
                        protagonist_exps_learner, protagonist_exps_memory, protagonist_exps_mu, protagonist_exps_logvar = self.discmodel(protagonist_exps_sb.obs, protagonist_exps_sb.action.to(self.device), protagonist_exps_memory.to(self.device) * antagonist_exps_sb.mask.to(self.device))
                    else:
                        antagonist_exps_learner = self.discmodel(antagonist_exps_sb.obs, antagonist_exps_sb.action.to(self.device))
                        demos_learner, demos_mu, demos_logvar = self.discmodel(demos_sb.obs, demos_sb.action)
                        protagonist_exps_learner = self.discmodel(protagonist_exps_sb.obs, protagonist_exps_sb.action.to(self.device))
                
                    demos_kld = kl_divergence(demos_mu, demos_logvar)
                    demos_kld = demos_kld.mean()
                    
                    antagonist_kld = kl_divergence(antagonist_exps_mu, antagonist_exps_logvar)
                    antagonist_kld = antagonist_kld.mean()

                    protagonist_kld = kl_divergence(protagonist_exps_mu, protagonist_exps_logvar)
                    protagonist_kld = protagonist_kld.mean()

                    
                    kld = (demos_kld + protagonist_kld + antagonist_kld) / 3.
                    bottleneck_loss = kld - self.i_c
                    self.beta = max(0, self.beta + self.alpha_beta * bottleneck_loss.detach())
